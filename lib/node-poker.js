...
function checkForEndOfRound(table) {
  ...
  if (table.players.length === 2) {
    if (table.players.some(function(p) {return p.folded;})) {
      //Evaluate each hand
      for (var j = 0; j < table.players.length; j += 1) {
        // This is a hack
        var hand = new Hand(['AS', 'AD', 'AH', 'AC', '2D']);
...
function checkForWinner(table) {
  ...
  for (i = 0; i < winners.length; i += 1) {
    var winnerPrize = table.game.pot / winners.length;
    ...
    if (table.game.roundBets[winners[i]] === 0) {
      ...
      table.gameWinners.push({
        ...
        amount: winnerPrize,
        hand: Object.assign(winningPlayer.hand, {holeCards: winningPlayer.cards}),
        chips: winningPlayer.chips
      });
    }
    console.log('player ' + table.players[winners[i]].playerName + ' wins ' + winnerPrize);
  }
...
function checkForBankrupt(table) {
  ...
  for (i = 0; i < table.players.length; i += 1) {
    if (table.players[i].chips === 0) {
      ...
      table.players[i].GetChips(table.maxBuyIn);
    }
  }
}
...
function progress(table) {
  ...
  if (table.game) {
    if (checkForEndOfRound(table) === true) {
      if (table.players.length === 2) {
        table.currentPlayer = table.game.bigBlindPlayer;
      } else {
      ...
      if (table.game.roundName === 'River') {
        ...
        checkForWinner(table);
        checkForBankrupt(table);
        table.eventEmitter.emit('showdown');
        table.initNewRound();
...
Table.prototype.NewRound = function() {
  ...
  if (this.players.length === 2 && this.game.roundName === 'Deal') {
